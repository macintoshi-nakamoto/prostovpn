...
Четыре части одного сервиса:

| Папка      | Что это                                                                    |
|------------|----------------------------------------------------------------------------|
| `backend/` | FastAPI: база, приём оплаты, админское API `/api/admin/*`, публичное `/api/v1/*` |
| `site/`    | Публичный сайт: тарифы, оплата, выдача доступа, личный кабинет              |
...
**Тарифы.** Цена в копейках, срок, сколько стран и устройств даёт тариф,
показывать ли его на сайте. Цена лежит на тарифе: из неё считается и счёт
на сайте, и ожидаемая часть календаря. В шаблонах сайта цен нет.
...
site/                   публичный сайт, без сборки и зависимостей
├─ index.html           лендинг

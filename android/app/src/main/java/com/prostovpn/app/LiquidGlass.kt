...
/**
 * Реализация эффекта Liquid Glass (iOS 26 `glassEffect`) для Compose.
 *
 * Фон экрана записывается в общий [GraphicsLayer] ([backdropSource]), а каждый стеклянный
 * элемент ([liquidGlass]) рисует этот слой под собой со сдвигом на свою позицию, пропуская его
 * через цепочку эффектов: насыщенность → размытие → AGSL-шейдер преломления по краю линзы
 * с лёгкой хроматической дисперсией. Поверх — тонировка и блик по контуру.
 *
 * Градация по версиям Android:
 *  - API 33+: полный эффект (размытие + преломление + дисперсия);
 *  - API 31–32: размытие + насыщенность без преломления;
 *  - ниже: полупрозрачный материал без сэмплинга фона (как fallback в iOS < 26).
 */
class BackdropState {
        ...
        .drawWithCache {
            ...
            onDrawWithContent {
                ...
                if (glassLayer != null && src != null) {
                    ...
                    // Fallback (API < 31): полупрозрачный материал, как в iOS < 26
                    drawRoundRect(

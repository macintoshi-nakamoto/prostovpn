"""Общие зависимости админского API: кто спрашивает и от чьего имени."""
...
def audit(
    ...
    Отдельным коммитом: запись в журнал не должна откатываться вместе с
    неудачной операцией — наоборот, попытка тоже интересна.
    """
    db.add(
        AuditLog(

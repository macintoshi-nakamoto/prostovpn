"""
Точки входа узлов: /api/admin/endpoints.

Что здесь намеренно ОТСУТСТВУЕТ — правка набора обфускации. Не потому, что её
забыли: смена H/S/J в секции [Interface] рвёт разом всех пиров интерфейса, и
никакая аккуратность применения от этого не спасает. Ротация набора устроена
иначе: завести новую точку входа, старую перевести в «слив», дождаться
переезда людей и погасить пустую.
"""
...
class EndpointOut(Schema):
    ...
    rev: int
    note: str | None
    # Набор обфускации показываем: администратор должен видеть, что у точек
    # входа он разный — ради этого всё и делалось. Секретов здесь нет.
...
class VlessIn(BaseModel):
    server_id: int
    listen_port: int
    # Донорские домены: клиент пишет их в SNI, и соединение выглядит походом
    # на этот сайт. Один донор на точку входа — ротация делается ВТОРОЙ точкой
    # входа, а не правкой живой: смена имён требует перезапуска демона и рвёт
    # все текущие сессии узла.
...
def create_endpoint(
    ...
    """
    Заводит точку входа в состоянии «черновик» — на узле её ещё нет.

    Набор обфускации генерируется здесь и потом не меняется никогда: на нём
    держится изоляция интерфейса.
    """
...
def apply_endpoint(
    ...
    """
    Поднимает точку входа на узле и открывает её для новых подключений.

    Идемпотентно: уже существующий на узле интерфейс не пересоздаётся — иначе
    повторное нажатие сменило бы набор обфускации под живыми пирами.
    """
    ...
    try:
        ...
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc)) from exc
    except Exception as exc:  # noqa: BLE001 — узел мог не ответить
...
def create_vless(
    ...
    """Заводит точку входа VLESS+Reality. На узел она попадает по «применить»."""
    server = db.get(Server, body.server_id)
    if server is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "сервер не найден")
    try:
        ...
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc)) from exc
    except Exception as exc:  # noqa: BLE001 — узел мог не ответить на x25519
        raise HTTPException(status.HTTP_502_BAD_GATEWAY, f"узел не ответил: {exc}") from exc
...
def sync_vless(
    ...
    """
    Переписывает конфиг xray на узле из состояния базы и перечитывает демон.

    Панель — единственный источник правды: всё, что дописали на узле руками,
    исчезнет. Иначе «что на узле» и «что в базе» разъедутся, и сверять их будет
    нечем.
    """
    endpoint = db.get(NodeEndpoint, endpoint_id)
...
def change_state(
    ...
    """
    Открыть для новых / перевести в слив / вывести из обращения.

    «Слив» — это и есть ротация набора: новых не селим, живые доезжают сами.
    """
    endpoint = db.get(NodeEndpoint, endpoint_id)
    if endpoint is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "точка входа не найдена")

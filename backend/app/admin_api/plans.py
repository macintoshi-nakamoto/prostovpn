...
def _apply(plan: Plan, body: schemas.PlanIn) -> None:
    """
    Переносит форму в тариф.

    Цена идёт через `set_price`: копейки для сайта и рубли для календаря
    прибыли обязаны меняться вместе, иначе счёт выставится по одной сумме,
    а в отчёте появится другая.
    """
    plan.name = body.name

"""
Автопродление для бота.

Бот ходит в панель админским токеном (см. utils/panel.py в prosto-bot) и
действует от имени конкретной учётки по её логину: показать состояние
автосписания, подключить и получить ссылку на привязку, отключить.
Ссылки и статусы — те же, что видит кабинет на сайте: источник один,
services/recurring, и витрины не могут разойтись.
"""

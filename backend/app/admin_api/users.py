...
def extend_subscription(
    ...
    try:
        ...
        if body.register_payment and price and price > 0 and not user.is_free:
            ...
            # Подарок пригласившему — здесь же, по тому же условию, что и
            # запись в кассу: деньги пришли. Раньше бонус жил только в
            # выдаче заказа, и оплата звёздами (она идёт этим путём, минуя
            # заказ) пригласившему не засчитывалась вовсе. Ручное продление
            # без денег сюда не попадает — там price пустой.

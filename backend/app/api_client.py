    ...
    for server, key in _serve_targets(db, user, device_id, background):
        ...
        out.append(
            ServerOut(
                ...
                config=config,
                # Основной порт обязан быть в списке.
                #
                # `alt_port_list()` его намеренно выбрасывает — он и так стоит
                # в конфиге. Но конфиг мы сами же и переписываем при подборе:
                # у человека без рукопожатия там окажется, скажем, 2408, и
                # план перебора на клиенте (порт из конфига + этот список)

...
class Settings(BaseSettings):
    ...
    # Сколько дней дарим пригласившему: за переход по ссылке и за первую
    # оплату приглашённого. Ноль отключает начисление, не ломая учёт.
    referral_join_days: int = 2
    referral_purchase_days: int = 5
    # Сколько «переходов» в сутки оплачиваются одному пригласившему. Дни за
    # переход даются до всякой оплаты, поэтому это самая дешёвая для накрутки
    # часть: потолок обрывает конвейер из одноразовых аккаунтов, живому
    # человеку он не мешает. Ноль — без ограничения.

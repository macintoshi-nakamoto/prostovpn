...
Особенность, которую нельзя пропустить: ЮKassa не подписывает уведомления.
Единственное, что она даёт, — список адресов, с которых их шлёт. Поэтому
подлинность здесь проверяется двумя рубежами:

1. адрес отправителя из опубликованного списка сетей;
2. повторный запрос платежа в API по его идентификатору — статус и сумму
   берём из ответа API, а не из тела уведомления.

Второй рубеж и есть настоящий: тело уведомления подделать может кто угодно,
кто угадал адрес эндпоинта и пролез мимо фильтра адресов, а вот ответ API
приходит по TLS и по нашим ключам магазина. Поэтому без ключей уведомление
не принимается вовсе: адрес отправителя приходит из заголовка прокси и
подделывается одной строкой, то есть сам по себе рубежом не является.
"""
...
class YooKassaProvider:
    ...
    def create_payment(self, order: "Order") -> PaymentSession:
        ...
        body = {
            ...
            # Метаданные возвращаются в уведомлении: по ним находим заказ,
            # даже если сопоставление по payment_id почему-то не сработало.
        ...
        try:
            response = httpx.post(
                ...
                json=body,
                auth=self._auth,
                # Ключ идемпотентности — идентификатор заказа. Повторное
                # нажатие «Оплатить» не создаст второй платёж на ту же сумму.
    ...
    def verify_webhook(
        ...
        # У возврата object — это возврат, а платёж лежит в payment_id.
        ...
        try:
            ...
            # Decimal на мусорной строке бросает InvalidOperation (это
            # ArithmeticError): без перехвата уведомление ушло бы в 500.
            raise WebhookRejected("сумма в уведомлении не разбирается") from exc
        order_id = (obj.get("metadata") or {}).get("order_id")

        # Главная проверка: спрашиваем сам платёж у API. Без ключей её
        # выполнить нечем, а фильтр адресов её не заменяет: client_ip
        # приходит из заголовка прокси, и подделать его может кто угодно.
        if not self._configured:

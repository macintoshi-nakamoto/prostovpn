...
# Соль для хэширования адресов клиента в ключах ограничителя частоты.
#
# Живёт ТОЛЬКО в памяти процесса и заново рождается при каждом запуске: на диск
# не пишется никогда. Смысл в этом. IP-адресов на свете перебираемо мало (v4 —
# четыре миллиарда), и по обычному хэшу утёкшая база разворачивается словарём.
# Со случайной солью, которой нет на диске, ключ ограничителя — это просто
# случайные байты: из дампа не узнать, кто и откуда заходил. Цена — перезапуск
# панели обнуляет счётчики частоты, и это ровно та цена, которую не жалко.
_RATE_SALT = secrets.token_bytes(32)


def ip_tag(ip: str | None) -> str:
    """
    Непрозрачная метка адреса для ключей ограничителя частоты.

    Не для хранения о человеке, а чтобы отличать один источник запросов от
    другого в пределах жизни процесса. Развернуть обратно в IP нельзя — соли
    нет на диске.
    """
    import hmac

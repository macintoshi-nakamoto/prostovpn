...
def _enforce_device_limit(db: OrmSession, user: User, current: Session) -> None:
    ...
    Устройство — это приложение. Вход в личный кабинет открывает сессию с
    platform="web", и до этой оговорки она была полноправным «устройством»:
    заняв последний слот, кабинет выкидывал телефон — человек заходил
    продлить подписку и этим же обрывал себе VPN. Веб-сессии в лимите не
    участвуют с обеих сторон: сами слот не занимают и кандидатами на
    отключение не становятся.
    """
    ...
    live = [
        s for s in user.live_sessions() if s.id != current.id and s.platform != "web"
    ]

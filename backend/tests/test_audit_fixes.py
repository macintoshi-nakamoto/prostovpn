"""
Регрессии на находки предпродакшн-аудита.

Каждый тест — сценарий потери денег или доступа, который аудит подтвердил
на живом коде. Ломаться они должны раньше, чем ломаются деньги.
"""
...
def test_refund_of_one_renewal_keeps_other_paid_days(client, fake_api):
    """Возврат второго продления не гасит дни первого — и не банит человека."""
    ...
    # Второе продление того же тарифа: та же строка подписки удлиняется.
...
def test_confirmed_after_refund_does_not_grant(client, fake_api):
    """Опоздавшее «оплачено» после возврата не воскрешает заказ."""
    order_id = _order_for(client, fake_api, "audit-late@example.com", "tx-late-1")

    # Возврат приходит раньше подтверждения (выдача упала, чарджбек догнал).
    r = _post_webhook(
        client,
        {
            "id": "tx-late-1",
...
def test_lost_subscription_charge_is_healed_by_retry(client, fake_api):
    """Списание, погибшее между claim и выдачей, доигрывает обходчик."""
    ...
    # Имитация краха: событие застолблено, обработка не дошла до конца.
    payload = {
        "id": "charge-retry-1",
        "amount": 199,
        "currency": "RUB",
    ...
    # Повтор доставки того же события — дубликат: провайдер уже не спасёт.
    r = _post_webhook(
        client,
        {
            "Id": "charge-retry-1",
...
def test_second_positive_payment_for_order_is_rejected_by_index(client, fake_api):
    ...
    with SessionLocal() as db:
        ...
        # Отрицательная строка (сторно) по тому же заказу — законна.
        db.add(
            Payment(
                user_id=original.user_id,
                order_id=order_id,

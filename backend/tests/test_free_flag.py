"""
Метка «бесплатный доступ».

Проверяются деньги, а не галка: продление бесплатника не пишет платёж даже
с нажатой галкой, прогноз выручки его не ждёт, а вот настоящая оплата через
провайдера в кассу попадает — реальные деньги важнее метки.
"""
...
def test_free_user_extension_writes_no_payment(client, auth):
    ...
    # Сняли метку — платёж снова пишется.
...
def test_free_user_is_not_expected_revenue(client, auth):
    ...
    # Продлим, чтобы подписка попала в окно ожиданий.
    r = client.post(
        f"/api/admin/users/{user_id}/extend",

"""
Приватность: о человеке в базе не остаётся следа, откуда он заходил.

Даже дамп базы не должен выдавать IP-адреса. Проверяем, что вход, регистрация
и заказ не пишут адрес никуда, а ключи ограничителя частоты — по хэшу со
случайной солью, который обратно в IP не развернуть.

Запуск: .venv/bin/python -m pytest tests/test_privacy.py -q
"""
...
def test_login_stores_no_ip(client):
    ...
    with SessionLocal() as db:
        ...
        # device_id — случайный идентификатор установки, он остаётся (не PII).
...
def test_order_stores_no_ip(client):
    ...
    # Заказ создаётся публичным API; проверяем, что колонка ip пуста у всех.
...
def test_ip_tag_is_not_reversible_without_salt():
    """Метка адреса — не сам адрес: разные адреса дают разные метки, а сам IP в ней не виден."""
    a, b = ip_tag("198.51.100.7"), ip_tag("203.0.113.9")
    assert a != b
    assert "198.51.100.7" not in a and "203.0.113.9" not in b
    # Детерминирована в пределах процесса (нужно ограничителю).

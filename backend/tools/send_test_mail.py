"""
Проверка почтового канала: отправляет одно письмо и говорит, что вышло.

Нужен, потому что «письма не приходят» выясняется обычно после оплаты, на
живом человеке. Скрипт бьёт тем же кодом, что и очередь доставки, — тем же
провайдером, тем же отправителем, — поэтому его успех означает, что дойдёт
и письмо с доступом.

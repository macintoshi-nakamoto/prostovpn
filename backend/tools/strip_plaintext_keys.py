"""
Шаг 1b перехода на шифрование приватников: вычистка открытого текста.

Что делает. У каждого живого ключа приватник должен быть уже зашифрован в
`private_key_enc` (это делает бэкфилл при старте панели, шаг 1a). Здесь мы
проверяем, что шифр читается и совпадает с открытым текстом в `config`, и
только тогда заменяем строку `PrivateKey = <ключ>` на плейсхолдер
...
Почему отдельным шагом, а не при старте. Это точка невозврата: после неё
приватник живёт ТОЛЬКО в шифре, и потеря PANEL_SECRETS_KEY = недоступность VPN
всему флоту. Запускать это должен человек, осознанно, после того как убедился,
что все пути отдают рабочий конфиг из шифра (канарейка).

Безопасно по построению: трогаем только ключи с проверенным roundtrip
(decrypt(enc) == открытый текст). Не сошлось — пропускаем и говорим об этом.
По умолчанию РЕЖИМ ПРОВЕРКИ (ничего не меняет), запись — только с --apply.

    cd /opt/prosto-vpn/backend
    cp panel.db /root/panel.db.before-strip-$(date +%F-%H%M)   # бэкап!
    set -a && . ./.env && set +a
    .venv/bin/python tools/strip_plaintext_keys.py            # сухой прогон
    .venv/bin/python tools/strip_plaintext_keys.py --apply    # запись
"""
...
from app import crypto, provisioning  # noqa: E402
from app.db import SessionLocal, init_db  # noqa: E402
from app.models import UserKey  # noqa: E402

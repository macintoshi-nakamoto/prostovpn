...
def start(file_id: str | None) -> list[dict]:
    ...
    return rich.screen(
        file_id,
        rich.paragraph(["Привет! ", rich.bold("Prosto"), " - интернет без блокировок."]),
        rich.spacer(),
        rich.paragraph(
            [
                rich.emoji("key"),
                " Сами решайте, что смотреть: быстрый и надёжный доступ ко всему, "
                "что закрыли. Ваш интернет - ваша свобода.",
            ]
        ),
    )

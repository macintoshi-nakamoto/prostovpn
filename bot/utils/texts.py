...
#
# На время проверки платёжной системы здесь стояли мягкие слова без
# «блокировок» и «без ограничений» — банк на верификации такие обещания
# отклоняет. 05.09.2026 по просьбе партнёра вернули человеческий язык;
# если проверка повторится, смягчать снова только эти строки, screens.start
# и BOT_DESCRIPTION в config/settings.py.
START_LEAD = "Один аккаунт на все устройства - вход по логину и паролю."

START_POINTS = (
    "Открывает всё, что закрыли: видео, соцсети, мессенджеры",
...
def start_text() -> str:
    return (
        "Привет! <b>Prosto</b> - интернет без блокировок.\n\n"
        f'{tg("key")} Сами решайте, что смотреть: быстрый и надёжный доступ '
        "ко всему, что закрыли. Ваш интернет - ваша свобода."
    )

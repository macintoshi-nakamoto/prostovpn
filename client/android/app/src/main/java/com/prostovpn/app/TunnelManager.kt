    ...
    */
    ...
    suspend fun connect(configText: String): Result = mutex.withLock {
        ...
        for (attempt in 1..ATTEMPTS) {
            ...
            через раз. Проверено безопасно: движок за это время сам повторит
            init, а мы просто не считаем эти секунды провалом.
            */
    ...
    companion object {
        ...
        /*
        Стратегия подключения переработана под потерю пакетов на мобильных
        сетях (жалобы с Huawei: «иногда подключается, иногда нет»). Сервер и
        обфускация тут ни при чём — проверено реальным подключением, 12 из 12
        рукопожатий проходят за полсекунды. «Иногда» — это когда мобильная
        сеть теряет или фильтрует пакеты рукопожатия.

        Раньше интерфейс пересоздавался каждые 20 секунд. Но движок сам
        ретрансмитит handshake init примерно раз в пять секунд, а пересоздание
        VpnService на Huawei заново расставляет маршруты и в этот момент само
        роняет пакеты. Получалось, что мы мешали движку в самый неудачный
        момент.

        Теперь: короткая стабилизация после поднятия (маршруты Huawei), потом
        ДОЛГОЕ первое окно — движок делает внутри него до десятка попыток init,
        не теряя маршруты. Пересоздание — крайняя мера на случай блокировки
        порта по DPI, и заходов больше (четыре), а суммарное терпение выше:
        подключиться за минуту лучше, чем сдаться за пятьдесят секунд.
        */

...
/**
 * Перебор портов трогает боевой конфиг, который потом уходит в туннель.
 *
 * Ошибка здесь не выглядит ошибкой: конфиг остаётся синтаксически верным, а
 * подключение просто перестаёт проходить — ровно тот симптом, ради которого
 * перебор и написан. Поэтому проверяем построчно и на настоящем конфиге.
 */
class EndpointsTest {
    ...
    fun `подмена порта не трогает остальное`() {
        ...
        // Всё, кроме одной строки, обязано остаться прежним: в конфиге есть
        // и другие числа после двоеточий и знаков «равно» — от MTU до ключей.
        assertEquals(

...
// Ради amnApp->networkManager(): общий на приложение пул соединений.
...
void CoreController::initControllers()
{
    ...
    // Вход по логину и паролю из панели. Сетевой менеджер берём общий,
    // приложенческий: свой означал бы второй пул соединений и вторые
    // настройки прокси.
    m_panelAuthController = new PanelAuthController(m_importCoreController, m_settings,
                                                    amnApp->networkManager(), this);
    setQmlContextProperty("PanelAuthController", m_panelAuthController);

    // Обновление приложения через нашу же панель: адрес берём у контроллера
    // входа, чтобы настройка была одна на оба.
    m_appUpdateController = new AppUpdateController(amnApp->networkManager(), this);
    m_appUpdateController->setPanelUrl(m_panelAuthController->panelUrl());
    connect(m_panelAuthController, &PanelAuthController::panelUrlChanged, m_appUpdateController,
            [this]() { m_appUpdateController->setPanelUrl(m_panelAuthController->panelUrl()); });
    setQmlContextProperty("AppUpdateController", m_appUpdateController);

    // Сохранённая сессия — приложение открывается сразу списком стран,
    // а не формой входа.
    m_panelAuthController->restoreSession();

    // Проверяем версию при запуске: кнопка обновления должна появиться сама,
    // без похода в настройки.
    m_appUpdateController->check();
}
...
void CoreController::initAppleController()
{
    ...
    QTimer::singleShot(0, this, [this]() { NexaVPN::toggleScreenshots(m_appSettingsRepository->isScreenshotsEnabled()); });
#endif
}
...
void CoreController::initSignalHandlers()
{
    ...
    #if NEXA_ENABLE_UPSTREAM_UPDATES && !defined(Q_OS_ANDROID) && !defined(Q_OS_IOS)
...
void CoreController::updateTranslator(const QLocale &locale)
{
    ...
    if (m_languageUiController) {
        emit websiteUrlChanged(QString::fromUtf8(NEXA_HOMEPAGE_URL));
    }
}

...
namespace
{
    ...
    const QLatin1String kInstallerRemoteFileNamePattern("NexaVPN_%1_windows_x64.exe");
    const QString kInstallerLocalPath = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/NexaVPN_installer.exe";
#elif defined(Q_OS_MACOS) && !defined(MACOS_NE)
    const QLatin1String kInstallerRemoteFileNamePattern("NexaVPN_%1_macos_x64.pkg");
    const QString kInstallerLocalPath = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/NexaVPN.pkg";
#elif defined(Q_OS_LINUX) && !defined(Q_OS_ANDROID)
    const QLatin1String kInstallerRemoteFileNamePattern("NexaVPN_%1_linux_x64.run");
    const QString kInstallerLocalPath = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/NexaVPN.run";
#endif
}
...
{
#if !NEXA_ENABLE_UPSTREAM_UPDATES
    logger.debug() << "Upstream update checks are disabled for this Алиса VPN build";
    return;
#endif
...
{
#if !NEXA_ENABLE_UPSTREAM_UPDATES
    logger.warning() << "Installer execution is disabled until Алиса VPN has a signed update manifest";
    return;
#endif

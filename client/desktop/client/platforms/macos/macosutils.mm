  ...
  if (@available(macOS 13, *)) {
    ...
    if (startAtBoot) {
      if (![[SMAppService mainAppService] registerAndReturnError: & error]) {
        logger.error() << "Failed to register Алиса VPN LoginItem: " << error.localizedDescription;
      } else {
        logger.debug() << "Алиса VPN LoginItem registered successfully.";
      }
    } else {
      if (![[SMAppService mainAppService] unregisterAndReturnError: & error]) {
        logger.error() << "Failed to unregister Алиса VPN LoginItem: " << error.localizedDescription;
      } else {

...
{
#if defined(Q_OS_IOS)
constexpr auto kIosBundleId = "com.nexavpn.client";
#else
constexpr auto kAndroidPackage = "com.nexavpn.client";
constexpr auto kAndroidStoreUrl = "https://play.google.com/store/apps/details?id=com.nexavpn.client";
#endif
} // namespace
#endif
...
void MarketplaceUpdateController::start()
{
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader, QByteArrayLiteral("NexaVPN"));
...
bool MarketplaceUpdateController::parseStoreVersion(const QByteArray &body, QString &version, QString &storeUrl)
{
    ...
    return !version.isEmpty() && !storeUrl.isEmpty();
  #else
...
void MarketplaceUpdateController::showUpdatePrompt(const QString &storeUrl)
{
    ...
    const QString message = tr("A new version of Алиса VPN is available.");

...
PageType {
    ...
    QtObject {
        ...
        // Пункт всегда на месте, но с обновлением он подписан иначе —
        // человек должен заметить его, не заходя внутрь.
        property string title: AppUpdateController.updateAvailable
                ? qsTr("Обновить приложение — версия %1").arg(AppUpdateController.latestVersion)
                : qsTr("Обновление приложения")
        readonly property string leftImagePath: "qrc:/images/controls/download.svg"
        property bool isVisible: true
        readonly property var clickedHandler: function() {
            PageController.goToPage(PageEnum.PageAppUpdate)
        }
    }
    ...
    QtObject {
        ...
        property string title: qsTr("Аккаунт")
        readonly property string leftImagePath: "qrc:/images/controls/settings.svg"
        // Пункт есть только у тех, кто вошёл по логину и паролю: в сборке
        // без панели ему нечего показывать.
        property bool isVisible: PanelAuthController.isLoggedIn
        readonly property var clickedHandler: function() {
            PageController.goToPage(PageEnum.PagePanelAccount)
        }
    }
    ...
    QtObject {
        ...
        property string title: qsTr("Share access")
        readonly property string leftImagePath: "qrc:/images/controls/share-2.svg"
        // Конфиг, выданный панелью, не отдаём наружу: в нём лежит личный
        // ключ, а доступом распоряжается администратор, а не клиент.
        property bool isVisible: ServersUiController.hasServerWithWriteAccess() && !PanelAuthController.isLoggedIn
        readonly property var clickedHandler: function() {
    ...
    QtObject {
        ...
        property string title: qsTr("About Алиса VPN")
        readonly property string leftImagePath: "qrc:/images/controls/nexa.svg"
        property bool isVisible: true
        readonly property var clickedHandler: function() {

...
PageType {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("Алиса VPN logs")
        ...
        readonly property var exportLogsHandler: function() {
            var fileName = ""
            if (GC.isMobile()) {
                fileName = "NexaVPN.log"
            } else {
                fileName = SystemController.getFileName(qsTr("Save"),
                                                        qsTr("Logs files (*.log)"),
                                                        StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/NexaVPN",
                                                        true,
                                                        ".log")
            }
            if (fileName !== "") {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("Алиса VPN service logs")
        ...
        readonly property var exportLogsHandler: function() {
            ...
            fileName = SystemController.getFileName(qsTr("Save"),
                                                    qsTr("Logs files (*.log)"),
                                                    StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/NexaVPN-service",
                                                    true,
                                                    ".log")
            if (fileName !== "") {
                PageController.showBusyIndicator(true)

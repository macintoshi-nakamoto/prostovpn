...
PageType {
    ...
    QtObject {
        ...
        readonly property var clickedHandler: function() {
            ...
            var descriptionText = qsTr("All installed VPN services will remain on the server.")
            var yesButtonText = qsTr("Continue")
            var noButtonText = qsTr("Cancel")
    ...
    QtObject {
        ...
        property bool isVisible: root.isServerWithWriteAccess
        readonly property string title: qsTr("Clear compatible VPN software from the server")
        readonly property string description: ""
        readonly property var tColor: AmneziaStyle.color.vibrantRed
        readonly property var clickedHandler: function() {
            var headerText = qsTr("Do you want to clear compatible VPN software from the server?")
            ...
            var yesButtonFunction = function() {
                if (ServersUiController.isDefaultServerCurrentlyProcessed() && ConnectionController.isConnected) {
                    PageController.showNotificationMessage(qsTr("Cannot clear VPN software from the server during an active connection"))
                } else {

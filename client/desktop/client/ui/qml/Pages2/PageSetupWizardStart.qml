...
PageType {
    ...
    Connections {
        ...
        function onServersUpdated(count) {
            if (count > 0) {
                PageController.showNotificationMessage(qsTr("Доступно стран: %1").arg(count))
            }
        }
    }

    function submitLogin() {
        if (PanelAuthController.busy) {
            return
        }

        var login = loginField.text.trim()

        // Пустые поля отправлять некуда: сервис ответит той же ошибкой,
        // только через сеть и через секунду.
        if (login === "") {
        ...
        PanelAuthController.login(login, passwordField.text)
    }
    ...
    FlickableType {
        ...
        ColumnLayout {
            ...
            BasicButtonType {
                ...
                enabled: !PanelAuthController.busy
                text: PanelAuthController.busy ? qsTr("Входим…") : qsTr("Войти")

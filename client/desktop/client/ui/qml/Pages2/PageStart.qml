...
PageType {
    ...
    Connections {
        ...
        function onGoToPageHome() {
            // Пока человек не вошёл в аккаунт, показывать нечего: страны
            // выдаёт панель, а без входа она не отдаёт ни одной.
            if (!PanelAuthController.isLoggedIn) {
        ...
        function onGoToPageSettings() {
            ...
            tabBarStackView.goToTabBarPage(PageEnum.PageSettingsApplication)
        }
    ...
    AlisaSidebar {
        id: sidebar
        objectName: "alisaSidebar"

        anchors.top: parent.top
        anchors.bottom: parent.bottom
        anchors.left: parent.left

        visible: tabBar.visible && GC.isDesktop()
        width: visible ? implicitWidth : 0

        tabs: [
            { title: qsTr("Подключение"), icon: "qrc:/images/controls/power.svg",
              handler: function() { tabBarStackView.goToTabBarPage(PageEnum.PageHome) } },
            { title: qsTr("Серверы"), icon: "qrc:/images/controls/globe-2.svg",
              handler: function() { tabBarStackView.goToTabBarPage(PageEnum.PageSettingsServersList) } },
            { title: qsTr("Настройки"), icon: "qrc:/images/controls/settings.svg",
              handler: function() { tabBarStackView.goToTabBarPage(PageEnum.PageSettingsApplication) } },
            { title: qsTr("Поддержка"), icon: "qrc:/images/controls/help-circle.svg",
              handler: function() { tabBarStackView.goToTabBarPage(PageEnum.PageSettingsAbout) } }
        ]
    }
    ...
    StackViewType {
        ...
        Component.onCompleted: {
            var pagePath
            if (!PanelAuthController.isLoggedIn) {
                // Первый экран приложения — вход по логину и паролю.
                tabBar.visible = false
    ...
    TabBar {
        ...
        height: (visible && GC.isMobile()) ? homeTabButton.implicitHeight + tabBar.topPadding + tabBar.bottomPadding : 0

        enabled: GC.isMobile() && !root.isControlsDisabled && !root.isTabBarDisabled
        ...
        TabImageButtonType {
            ...
            // Добавление сервера вручную — обход учётной записи: человек
            // вставлял бы чужой ключ и получал доступ мимо подписки.
            // Серверы приходят из аккаунта, поэтому кнопки нет.
            visible: !PanelAuthController.isLoggedIn
            width: PanelAuthController.isLoggedIn ? 0 : undefined

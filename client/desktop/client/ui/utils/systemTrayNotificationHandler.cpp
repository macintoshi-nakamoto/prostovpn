...
{
    websiteUrl = QString::fromUtf8(NEXA_HOMEPAGE_URL);
    m_systemTrayIcon.show();
    connect(&m_systemTrayIcon, &QSystemTrayIcon::activated, this, &SystemTrayNotificationHandler::onTrayActivated);

    m_trayActionShow =  m_menu.addAction(tr("Show") + " " + APPLICATION_DISPLAY_NAME, this, [this](){
        emit raiseRequested();
    });
    m_menu.addSeparator();
    ...
    m_trayActionVisitWebSite->setVisible(!websiteUrl.isEmpty()
                                         && !websiteUrl.contains("amnezia-vpn/amnezia-client"));

    // Quit action: disconnect VPN first on macOS NE, else quit directly
    m_trayActionQuit = m_menu.addAction(tr("Quit") + " " + APPLICATION_DISPLAY_NAME,
                                       this,
                                       [&](){ qApp->quit(); });
...
{
    m_trayActionShow->setText(tr("Show") + " " + APPLICATION_DISPLAY_NAME);
    ...
    m_trayActionQuit->setText(tr("Quit")+ " " + APPLICATION_DISPLAY_NAME);
}

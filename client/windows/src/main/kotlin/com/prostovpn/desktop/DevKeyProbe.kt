...
/**
 * Прогон произвольного ключа через весь путь приложения: gradle keyprobe -Pkey=<файл>
 *
 * Показывает, что именно уйдёт движку туннеля, — без этого разница между
 * «ключ разобран не так» и «сервер не отвечает» неразличима. Ключ берётся
 * из файла и никуда не сохраняется: в нём приватные ключи.
 */
fun main(args: Array<String>) {
    ...
    // Сверяем набор ключей с тем, что примет движок: лишний ключ валит
    // конфигурацию устройства целиком

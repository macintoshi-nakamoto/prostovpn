...
/**
 * Проверка входа в панель без запуска окна.
 *
 * Собирать интерфейс ради того, чтобы убедиться, что логин и пароль
 * доходят до сервера, — долго и неудобно. Запуск:
 *
 *   ./gradlew panelcheck -Ppanel=http://127.0.0.1:8000 -Plogin=... -Ppass=...
 */
object PanelCheck {
        ...
        runBlocking {
            PanelApi.login(login, password)
                .onSuccess { session ->
                    ...
                    // Второй запрос — уже по токену, как приложение делает
                    // при каждом запуске.

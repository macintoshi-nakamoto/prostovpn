...
func watchStopEvent(confPath string) {
	...
	// ERROR_ALREADY_EXISTS — не ошибка: CreateEvent возвращает живой хэндл
	// уже существующего события. Так бывает, когда прежняя служба ещё не
	// умерла (или приложение держит хэндл со своей стороны), и выходить
	// здесь значило остаться без самоостановки вовсе: /down выставлял бы
	// сигнал, которого никто не ждёт, ждал 20 секунд и эскалировал до
	// снятия службы через UAC — ровно то, чего событие должно избегать.

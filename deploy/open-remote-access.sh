...
if [[ "${1:-}" == "--close" ]]; then
    ...
    # Правила ufw для этого порта снимаем все, кем бы они ни были заведены.
    while ufw status numbered | grep -qE "^\[[ 0-9]+\].*$PORT"; do
        num=$(ufw status numbered | grep -oE "^\[[ 0-9]+\]" | head -1 | tr -d '[] ')

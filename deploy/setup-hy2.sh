...
# Пользователей не хранит — на каждое соединение спрашивает панель
# (POST /api/v1/hy2/auth), паролем служит UUID VLESS-учётки человека на
# этом узле. Сертификат самоподписанный: клиент идёт с insecure=1, а DPI
# видит только SNI из ссылки. На HTTP/3-зонды отвечает как донор Reality.
#

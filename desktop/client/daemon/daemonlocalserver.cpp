...
constexpr const char* TMP_PATH = "/tmp/nexavpn.socket";
constexpr const char* VAR_PATH = "/var/run/nexavpn/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("nexavpn")) {
    logger.debug() << "/var/run/nexavpn seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("nexavpn")) {
    logger.warning() << "Failed to create /var/run/nexavpn";
    return TMP_PATH;
  }

  if (chmod("/var/run/nexavpn", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/nexavpn";
    return TMP_PATH;
  }

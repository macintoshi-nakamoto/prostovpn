...
if dscl . -read "/Groups/$SERVICE_GROUP" >/dev/null 2>&1; then
  ...
  run_cmd dscl . -create "/Groups/$SERVICE_GROUP" RealName "Nexa VPN Service Group"
fi
...
run_cmd osascript -e 'tell application "NexaVPN" to quit' || true

...
/// Клиент UAPI — того самого текстового протокола, которым настраивается
/// движок. Сокет он кладёт в /var/run/amneziawg/<интерфейс>.sock.
///
/// Каталог именно amneziawg, а не wireguard: у форка он свой, и попытка
/// ждать сокет по «общеизвестному» пути заканчивается тем, что туннель
/// поднимается, но настроить его нечем.
enum UAPI {
    ...
    /// Отправляет запрос и возвращает ответ целиком.
    static func exchange(interfaceName: String, request: String) throws -> String {
        ...
        // Таймаут обязателен: без него зависший движок подвешивает и хелпер,
        // а вместе с ним весь интерфейс приложения.
        ...
        while true {
            ...
            // Ответ UAPI заканчивается пустой строкой
    ...
    /// Состояние туннеля со стороны движка.
    ///
    /// Складываем счётчики по всем пирам и берём самое свежее рукопожатие:
    /// пир в конфиге обычно один, но конфиг с несколькими — не ошибка.

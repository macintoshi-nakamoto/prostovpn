...
/*
 Привилегированный хелпер Prosto VPN.

 Работает от root под launchd и делает ровно три вещи: поднимает туннель,
 снимает его и рассказывает о его состоянии. Никаких команд из конфигурации
 он не выполняет — иначе сервер, приславший конфиг, получал бы root на
 машине пользователя.
 */
...
// После падения хелпера или перезагрузки туннеля уже нет, а подменённый DNS
// и блокирующие маршруты остаться могли. Возвращаем систему в исходное
// состояние до того, как примем первую команду.
SystemNetwork.restoreDNS()
SystemNetwork.removeBlackhole()

// MARK: - Кто имеет право командовать

/// Uid владельца, записанный установщиком. Хелпер один на машину, а
/// управлять им должен только тот, кто его поставил, — иначе любой
/// локальный пользователь смог бы направить чужой трафик через свой сервер.
let ownerUID: uid_t? = {
...
private func peerUID(of fd: Int32) -> uid_t? {
    ...
    // SOL_LOCAL = 0, LOCAL_PEERCRED = 1 — заголовочные константы Darwin,
    // в Swift они не импортируются.
...
private func serve(_ fd: Int32) {
    ...
    // Запрос — одна строка JSON. Ограничение на длину защищает от клиента,
    // который открыл соединение и льёт байты без конца.
...
private func listenSocket(at path: String) -> Int32 {
    ...
    // Права на сам файл сокета широкие, но подключившегося мы всё равно
    // проверяем по uid: так понятная ошибка «доступ запрещён» доходит до
    // клиента вместо молчаливого отказа на уровне файловой системы.
    chmod(path, 0o666)
...
while true {
    ...
    // Каждое соединение обслуживается отдельно: «подключить» занимает
    // секунды, и опрос состояния не должен ждать его завершения.

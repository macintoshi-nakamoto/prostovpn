...
/// Переключатель на системном стекле.
///
/// Дорожка — не залитая капсула, а само стекло: при включении оно
/// подкрашивается акцентом, а не закрывается сверху плашкой. Бегунок на
/// ходу вытягивается в каплю и возвращает круглую форму на месте — так
/// движение читается как жидкость, а не как сдвиг картинки.
struct GlassToggleStyle: ToggleStyle {
    ...
    private struct Surface: View {
        ...
        private let trackHeight: CGFloat = 27
        private let knobSize: CGFloat = 21

        private var knobWidth: CGFloat {
            stretched && !reduceMotion ? knobSize + 7 : knobSize
        }
        ...
        var body: some View {
            Button {
                ...
                ZStack(alignment: configuration.isOn ? .trailing : .leading) {
                    // Прозрачная подложка задаёт размер: сама дорожка — стекло,
                    // которое подставляется под неё сзади.
                    Color.clear
                ...
                .frame(width: trackWidth, height: trackHeight)
                // Дорожка подкрашена всегда. Чистое стекло на почти чёрном
                // фоне не видно вовсе, и выключенный переключатель выглядел
                // одиноким кружком — непонятно ни что это, ни где он стоит.
                .glassCapsule(
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(Color.white.opacity(configuration.isOn ? 0.2 : 0.14), lineWidth: 1)
                }
            ...
            .buttonStyle(.plain)
            .onHover { hovering = $0 }
            .animation(.spring(response: 0.34, dampingFraction: 0.62), value: configuration.isOn)
            .animation(.spring(response: 0.26, dampingFraction: 0.55), value: knobWidth)
            .animation(Theme.spring(0.18), value: hovering)
        ...
        /// Вытягивание держится только на время перелёта: постоянно
        /// растянутый бегунок выглядел бы сломанным.
        private func squish() {

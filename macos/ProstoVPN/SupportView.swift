...
/// Страницы указываем с расширением — так они открываются и когда сайт отдаёт
/// nginx, и когда его раздаёт сама панель.
enum Site {
    static let address = "https://prostovpn.cc"
    /// Поддержка — тот же бот, что оформляет подписку и выдаёт доступ.
    /// Отдельный аккаунт человека рядом с ним только заставлял выбирать,
    /// куда писать.

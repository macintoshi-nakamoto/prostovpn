...
/*
 Разбор wg-quick и сборка полезной нагрузки UAPI.

 Панель отдаёт обычный wg-quick с полями обфускации AmneziaWG (Jc, Jmin,
 S1, H1…). Движок настраивается не файлом, а протоколом UAPI, где ключи
 записаны в hex, а не в base64, — этот перевод и есть основная работа
 здесь.
 */

public struct WGPeer: Equatable, Sendable {
    public var publicKey: String              // base64, как в конфиге
...
public struct WGConfig: Equatable, Sendable {
    ...
    public var addresses: [String] = []       // 10.8.1.2/32
    public var dns: [String] = []
    public var mtu: Int = 1280
    public var listenPort: Int?
    /// Параметры обфускации в том виде, в каком их ждёт UAPI: ключ уже
    /// приведён к нижнему регистру (jc, jmin, s1, h1, i1…).
...
public enum WGQuick {
    ...
    /// Поля обфускации AmneziaWG. Всё, что вне списка, движку не отдаём:
    /// незнакомый ключ он считает ошибкой и обрывает настройку целиком.
    ...
    public static func parse(_ text: String) throws -> WGConfig {
        ...
        for rawLine in text.components(separatedBy: .newlines) {
            // Комментарии режем до разбора: «Endpoint = 1.2.3.4:51820 # прод»
            // иначе уедет в значение и порт перестанет разбираться.
    ...
    private static func applyInterface(key: String, value: String, to config: inout WGConfig) throws {
        ...
        case "dns":
            // В DNS иногда пишут и домен поиска — берём только адреса,
            // networksetup домен всё равно не принимает.
        ...
        case "table", "preup", "postup", "predown", "postdown", "savedconfig", "fwmark":
            // wg-quick-специфичные поля: сеть настраивает хелпер, а произвольные
            // команды из конфига сервера выполнять от root нельзя.
            break
        default:
    ...
    /// Строка настройки для движка.
    ///
    /// `resolved` — уже разрешённые адреса эндпоинтов: движок принимает
    /// только `ip:port`, имена он резолвить не умеет.
    public static func uapiPayload(_ config: WGConfig, resolvedEndpoints resolved: [String: String]) -> String {
        ...
        // Полная замена пиров: конфиг — источник истины, остатки от прошлого
        // подключения не должны продолжать жить.
    ...
    /// base64-ключ WireGuard → hex, как его ждёт UAPI. nil, если это не ключ.
    ...
    /// Хост и порт из строки `Endpoint`. IPv6 приходит в скобках.

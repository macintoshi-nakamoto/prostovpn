...
/**
 * Сетка месяца: в каждой клетке число и деньги этого дня.
 *
 * Факт и ожидание показываются разными строками, а не суммой: по прошедшим
 * дням это уже полученные деньги, по будущим — предположение, и складывать
 * их в одну цифру значит выдавать прогноз за выручку.
 */
export function MonthGrid({ data, currency, selected, onSelect }) {
  if (!data) return null;

  // Первый день месяца может быть любым днём недели — добиваем пустыми
  // клетками, иначе числа съедут на неправильные колонки.
        ...
        {cells.map((day, index) => {
          ...
          return (
            <button key={day.date} className={classes} onClick={() => onSelect(day)} type="button">
              {/* Заливка по величине дня: сильные дни видно, не читая цифр. */}
              {total > 0 && (
                <span
                  className="cal-fill"

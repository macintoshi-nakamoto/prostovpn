...
export function ServerModal({ server, onClose, onSaved }) {
  ...
  const invalid =
    !form.name.trim() ||
    !form.host.trim() ||
    (isSsh && !form.awgTemplate.trim()) ||
    (!isSsh && !form.sharedConfig.trim());
        ...
        {isSsh ? (
          <>
            ...
            <Field
              ...
              hint="Обязательны поля {private_key} и {address} — панель подставит их для каждого клиента"
            >
              <textarea
          ...
          <Field
            label="Общий конфиг"
            hint="Ссылка vpn:// или текст wg-quick. Его получат все — отозвать доступ одному человеку будет нельзя."
          >
            <textarea
              className="gd-textarea"
              style={{ minHeight: 160 }}

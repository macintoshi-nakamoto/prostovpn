 ...
 * Создание клиента.
 *
 * Логин и пароль по умолчанию придумывает панель: доступы, набранные
 * руками, всегда слабее, а вводить их всё равно не человеку, а приложению.
 */
export function CreateUserModal({ plans, onClose, onCreated }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Сначала показываем доступы, и только после закрытия отдаём наверх:
      // иначе пароль исчезнет вместе с окном, не успев попасть к человеку.
      setCreated(result);
    } catch (err) {

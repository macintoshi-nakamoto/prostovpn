...
def login(body: schemas.LoginRequest, db: OrmSession = Depends(get_db)) -> schemas.LoginResponse:
    try:
        admin, token, expires_at = services.authenticate_admin(db, body.login, body.password)

...
приходит по TLS и по нашим ключам магазина. Если ключи не заданы (панель
ещё не подключена), остаётся только первый рубеж — и об этом пишется
предупреждение.
"""
...
class YooKassaProvider:
    ...
    def verify_webhook(
        ...
        # Главная проверка: спрашиваем сам платёж у API.
        confirmed = self._fetch_payment(payment_id) if self._configured else None
        if confirmed is not None:
            ...
            log.warning(
                "ключи ЮKassa не заданы: уведомление принято только по адресу отправителя"
            )

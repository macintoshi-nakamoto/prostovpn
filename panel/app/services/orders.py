...
def public_plans(db: OrmSession) -> list[Plan]:
    """
    Тарифы для витрины: включённые, публичные и с ненулевой ценой.

    Цена больше нуля — не придирка. Бесплатный тариф на витрине показался бы
    как «0 ₽», а нажатие на него упёрлось бы в отказ создать заказ: продать
    через платёжный сервис нечего. Такие тарифы существуют для ручной выдачи
    из панели, и место им там.
    """
    return list(
        db.scalars(
            select(Plan)
...
def fulfil(db: OrmSession, order: Order, manual_by: int | None = None) -> Fulfilment:
    ...
    коммита.
    """
    if order.status == OrderStatus.PAID.value and order.user_id:
...
def refund(db: OrmSession, order: Order, reason: str = "возврат платежа") -> None:
    """
    Возврат или оспаривание: доступ снимается, пиры уходят с узлов.

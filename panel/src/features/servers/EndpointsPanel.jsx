 ...
 * Точки входа узла.
 *
 * Одна строка — один слушающий сокет: awg-интерфейс со своим набором
 * обфускации или inbound VLESS. Набор показывается целиком, и это не отладка:
 * администратор должен видеть, что у интерфейсов он РАЗНЫЙ — ради этого вся
 * фаза и делалась.
 *
 * Правки набора здесь нет и не будет. Смена H/S/J на живом интерфейсе рвёт
 * разом всех его пиров; ротация — это новая точка входа, старая в «слив».
 */
...
function Obfuscation({ values }) {
  ...
  // Порядок как в конфиге узла — чтобы строку из панели можно было сверить
  // глазами с тем, что реально стоит на машине.
  return (
...
function VlessModal({ serverId, onClose, onCreated }) {
  // Донор по умолчанию — крупный чужой сайт: соединение выглядит походом
  // именно на него. Один донор на точку входа: Reality пересылает исходный
  // ClientHello на dest, и зонд с чужим именем получил бы не тот сертификат.

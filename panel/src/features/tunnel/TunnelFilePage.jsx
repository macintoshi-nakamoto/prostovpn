 ...
 *
 * Экран есть, потому что этот список меняется чаще всего остального в
 * сервисе — банк переехал на новый домен, госуслуги добавили поддомен, —
 * и менять его должно быть не сложнее, чем цену тарифа. Ни выкладки на
 * сервер, ни правки конфигов: панель кладёт версию в базу, а кабинет, бот
 * и приложения с этого момента отдают её.
 *
 * История версий не для красоты: список легко испортить одной строкой, и
 * тогда у людей молча перестают открываться банки. Откат на прежнюю версию
 * — одна кнопка.
 */
 ...
 *
 * Файл читаем прямо в браузере и отправляем текстом. Так же работает правка
 * руками: поменять одну строку в списке, не выгружая и не загружая файл
 * заново, — обычное дело, и ради этого держать отдельный экран незачем.
 */
function UploadModal({ onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    // Проверяем разбор здесь, а не после сохранения: битый JSON приложение
    // молча не применит, и человек будет думать, что список обновился.
    try {
      JSON.parse(content);
    } catch (err) {

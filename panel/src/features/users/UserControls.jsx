...
/**
 * Блок управления пользователем — то, ради чего открывают карточку.
 *
 * Каждое действие сразу отдаёт обновлённую карточку, поэтому состояние
 * переключателей не расходится с сервером даже при отказе.
 */
export function UserControls({ user, plans, onResult, onDeleted }) {
  ...
  const resetPassword = async () => {
    ...
    try {
      ...
      onResult(fresh);
      // Пароль показывается ровно один раз — сразу после смены.

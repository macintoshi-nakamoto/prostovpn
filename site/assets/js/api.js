/* Разговор с панелью.
   ...
   Один модуль на все страницы: адреса методов и разбор ошибок должны
   лежать в одном месте, иначе на четвёртой странице появится свой формат
   сообщения об ошибке.

   Токен кабинета живёт в localStorage. Не в куке: у публичного API нет
   сессий на куках намеренно — эндпоинт вебхука обязан быть без кук, а
   держать два разных механизма на одном домене значит однажды случайно
   отправить куку туда, где её не ждут. */
...
export function getToken() {
  try {
    ...
    // Приватный режим в некоторых браузерах запрещает хранилище. Кабинет
    // всё равно работает — просто до перезагрузки страницы.
    return null;
  }
}
...
/** FastAPI отдаёт ошибку строкой в detail, а ошибку валидации — списком. */
...
/* ── Форматирование ──────────────────────────────────────────────────────
   Деньги приходят копейками и целым числом. Делим на сто ровно здесь, в
   одном месте: цена, посчитанная в вёрстке, рано или поздно разойдётся с
   той, по которой выставлен счёт. */

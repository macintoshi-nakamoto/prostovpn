/* Страница успеха.
   ...
   Важное: попадание сюда ничего не доказывает. Этот адрес можно набрать
   руками, и человек, который так сделает, не должен увидеть чужую учётку.
   Поэтому страница ничего не «выдаёт» — она спрашивает у сервера статус
   заказа и показывает только то, что он вернул.

   Ждём полторы минуты. Дальше честно говорим, что доступ придёт письмом:
   спиннер, который крутится вечно, читается как поломка, а зависший
   вебхук — не поломка, а обычная задержка платёжного сервиса. */
...
function fillCredentials(status) {
  ...
  if (status.password) {
    ...
    // Продление: пароль не менялся, и показывать его заново незачем.
    passwordRow.hidden = true;
  }

  // Без точки в конце: русская локаль уже дописывает «г.», и вторая точка
  // подряд выглядит опечаткой.
...
async function poll() {
  ...
  while (Date.now() - startedAt < WAIT_LIMIT_MS) {
    ...
    try {
      ...
      // Сеть моргнула — не повод пугать человека, у которого деньги ушли.
      await sleep(delay);

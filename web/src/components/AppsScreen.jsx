 ...
 * Скачать приложение.
 *
 * Один вопрос — какое устройство, — и дальше то, что на него ставится. Везде,
 * кроме iPhone, это только наше приложение: своё есть под все эти системы, и
 * предлагать рядом чужие значит заставлять выбирать там, где выбора нет.
 * На iPhone своего приложения нет — там список сторонних.
 *
 * Ключей здесь нет намеренно: они живут в своём разделе, где их выпускают и
 * удаляют. Раньше эти два дела стояли на одном экране, и человек, пришедший
 * просто скачать программу, попадал в выпуск ключей.
 */
...
export function AppsScreen({ open, onClose, onKeys }) {
  ...
  // Своё приложение есть везде, кроме iPhone, и на этих устройствах оно
  // единственное в списке: три чужих рядом с ним только сеют сомнение, каким
  // ставить. На iPhone своего приложения нет — там список остаётся.
  const list = ORDER.map((id) => ({ id, ...APPS[id] }))
    .filter((one) => one.on[device])
    .filter((one) => device === "ios" || one.kind === "ours");
  ...
  return (
    ...
    >
      <div className="su">
        ...
        {/* В нашем приложении ключ не нужен — там вход по логину, поэтому и
            подпись у карточки другая: ключи пригодятся только со сторонним. */}
        <AppCard
          icon={IC_KEY}
          title={t("su.keysTitle")}
          text={t(device === "ios" ? "su.keysLead" : "su.keysLeadOurs")}
        >
          <button
            type="button"

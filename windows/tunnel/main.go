...
func watchStopEvent(confPath string) {
	...
	// Ручной сброс: приложение может выставить событие раньше, чем мы дошли
	// до ожидания, и сигнал не должен потеряться.
	event, err := windows.CreateEvent(sa, 1, 0, name16)
	if err != nil {
		return
	}
